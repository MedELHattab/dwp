...
const InputWithLabel = ({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      {label && (
        <Label htmlFor={name} className="text-black text-end">
          {label}
        </Label>
      )}
      <div className="">
        <div className="relative">
          <Input
            ...
            } border-gray-600 placeholder:text-gray-400 hover:bg-dark/20 h-10`}
            value={value}
            onChange={onChange}
            type={typeInput}
            id={name}
          ...
          {type === "password" && (
            <>
              {typeInput === "password" && (
                <Eye
                  className="w-4 h-4 absolute right-2 top-1/2 bottom-1/2 -translate-y-1/2"
